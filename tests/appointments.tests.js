...
describe("Appointments Controller", () => {
  beforeEach(async () => {
    ...
    // Crear una dirección para el médico
    const doctorAddress = new Addresses({
      doctor: userId,
      street: "Calle del Médico",
      streetNumber: 123,
      floor: 5,
      country: "Argentina",
      province: "Buenos Aires",
      neighborhood: "Barrio Médico",
      zipCode: 1234,
    });
    await doctorAddress.save();
    const addressId = doctorAddress._id;
    ...
    const appointments = [
      {
        date: "2024-07-20",
        timeOfAppointment: "15:30",
        status: "Pendiente",
        cancelReason: null,
        patient: patients[0]._id,
        doctor: newUser._id,
        address: addressId, // Reemplaza con el ID de la dirección correspondiente
        service: service._id, // Reemplaza con el ID del servicio creado
    ...
    await Appointments.insertMany(appointments);
  });
  ...
  describe("GET api/patients/", () => {
    it("debería devolver un error si el ", async () => {
      const appointment = await Appointments.findOne();
      const id = appointment._id.toString();

      const res = await chai.request(app).get(`/api/appointments/${id}`);
